...
def vacuum_clean():
    ...
    while not rospy.is_shutdown():
        ...
        # Check if the turtle is close to hitting a wall
        if turtle_pose.x < 2:
            # Move away from the wall in the x direction
            velocity_msg.linear.x = 0.5
            velocity_msg.angular.z = -0.5
        elif turtle_pose.y < 2:
            # Move away from the wall in the y direction
            velocity_msg.linear.x = 0.5
            velocity_msg.angular.z = 0.5
        else:
            # Move the turtle in the chosen direction
            velocity_msg.linear.x = 0.5
            velocity_msg.angular.z = direction * 0.5
