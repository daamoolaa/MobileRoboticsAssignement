...
def vacuum_clean_turtle2():
    # Set up publisher to send velocity commands for turtle2
    pub2 = rospy.Publisher('/turtle2/cmd_vel', Twist, queue_size=10)
    rospy.init_node('vacuum_clean_turtle2', anonymous=True)
    rate = rospy.Rate(10) # 10 Hz

    # Subscribe to turtle2's pose to get its position
    turtle2_pose = Pose()
    def pose_callback2(pose_message):
        turtle2_pose.x = pose_message.x
        turtle2_pose.y = pose_message.y
    rospy.Subscriber("/turtle2/pose", Pose, pose_callback2)

    # Set the goal position for turtle2
    goal_x = 11
    goal_y = 11

    # Control loop to move
    while not rospy.is_shutdown():
        velocity_msg2 = Twist()

        # Calculate the angle between the turtle and the goal
        angle = math.atan2(goal_y - turtle2_pose.y, goal_x - turtle2_pose.x)

        # Check if the turtle has reached the goal
        if abs(turtle2_pose.x - goal_x) < 0.1 and abs(turtle2_pose.y - goal_y) < 0.1:
            # Choose a new goal
            goal_x = 11 * random.random()
            goal_y = 11 * random.random()
        else:
            # Move towards the goal
            velocity_msg2.linear.x = 5
            velocity_msg2.angular.z = 20 * (angle - turtle2_pose.theta)
             # Check if turtle1 is about to hit a wall and override movement
            if turtle2_pose.x < 0.5 or turtle2_pose.x > 11.5 or turtle2_pose.y < 0.5 or turtle2_pose.y > 11.5:
                velocity_msg2.linear.x = -1
                velocity_msg2.angular.z = 20

        pub2.publish(velocity_msg2)
        rate.sleep()

if __name__ == '__main__':
    try:
        vacuum_clean_turtle2()
    except rospy.ROSInterruptException:
        pass
